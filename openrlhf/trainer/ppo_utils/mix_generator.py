...
class SamplesGenerator:
    ...
    def generate_eval_samples(self, **generate_kwargs) -> List[Experience]:
        ...
        try:
            while True:
                experiences, _, exhausted = self._generate_vllm_full(
    ...
    def generate_train_samples(self, **generate_kwargs) -> Tuple[List[Experience], Optional[float], int, bool]:
        ...
        if len(self._sample_buffer) < chunk_size and self._dataloader_iter is not None:
            ...
            experiences, prompts_consumed, dl_exhausted = self._generate_vllm_proxy(
    ...
    def _generate_vllm_proxy(
        ...
        prompts_proxy, _, labels, images, exhausted = _collect_prompt_batch(dataloader_iter, num_prompts)
        if not prompts_proxy:
            return [], prompts_consumed, True

        target_num_prompts = len(prompts_proxy)
        pending_refs = self._dispatch_prompts_to_vllm(prompts_proxy, labels, images=images, **generate_kwargs)
        ...
        while pending_refs:
            ...
            for ref in ready_refs:
                ...
                if experiences:
                    ...
                    new_prompts_proxy, _, new_labels, new_images, exhausted = _collect_prompt_batch(dataloader_iter, 1)
                    prompts_consumed += len(new_prompts_proxy)
                    if exhausted and not new_prompts_proxy:
                        for remaining_ref in pending_refs:
                            ray.cancel(remaining_ref)
                        return [], prompts_consumed, True
                    if new_prompts_proxy:
                        new_refs = self._dispatch_prompts_to_vllm(
                            new_prompts_proxy, new_labels, images=new_images, **generate_kwargs
                        )
                        pending_refs.extend(new_refs)

        return accepted_experiences, prompts_consumed, exhausted
    
    def _generate_vllm_full(
        self, dataloader_iter, num_prompts: int, dynamic_filtering, **generate_kwargs
    ) -> Tuple[List[Experience], int, bool]:
        """Generate a batch of Experiences with optional reward filtering.
        ...
        _, prompts_full, labels, images, exhausted = _collect_prompt_batch(dataloader_iter, num_prompts)
        if not prompts_full:
            return [], prompts_consumed, True

        target_num_prompts = len(prompts_full)
        pending_refs = self._dispatch_prompts_to_vllm(prompts_full, labels, images=images, **generate_kwargs)
        prompts_consumed += target_num_prompts
        ...
        while pending_refs:
            ...
            for ref in ready_refs:
                ...
                if experiences:
                    ...
                    # Dispatch replacement for filtered prompt.
                    _, new_prompts_full, new_labels, new_images, exhausted = _collect_prompt_batch(dataloader_iter, 1)
                    prompts_consumed += len(new_prompts_full)
                    if exhausted and not new_prompts_full:
                        for remaining_ref in pending_refs:
                            ray.cancel(remaining_ref)
                        return [], prompts_consumed, True
                    if new_prompts_full:
                        new_refs = self._dispatch_prompts_to_vllm(
                            new_prompts_full, new_labels, images=new_images, **generate_kwargs
                        )
                        pending_refs.extend(new_refs)
