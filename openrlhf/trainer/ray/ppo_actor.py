...
class ActorPPOTrainer(ABC):
    ...
    def training_step(self, experience: Experience, kl_ctl: float, step: int) -> Dict[str, float]:
        ...
        # print("advantages", advantages.shape) # (batch-size, sequence-len - 1)
        ...
        print("tmp", action_log_probs[target_index: target_index + 1].detach()[action_mask[target_index: target_index + 1] == 1].shape)
        print("tmp", action_log_probs_full[action_mask_full == 1].shape)
